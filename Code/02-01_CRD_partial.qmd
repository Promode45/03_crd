...
- contrast type, - sum-of-squares type - deciding how to extract and display model means and pairwise comparisons
...
# b) EDA tables - exploratory data analysis
...
why blocks should be categorical and not numerical? For anova, treatments must be categorical (vector) and not numerical.
...
```{r n boxplot}
crd_dfw |> 
  ggplot(aes(nrate_kgha, yield_kgha, color = nrate_kgha)) +
  geom_boxplot() +
  geom_jitter() +
...
In type 1, order of the terms(factors) matter unlike in type 3.
...
-   Residual normality (discuss this!) - residuals changes with the model selected, so checking residual normality is paramount.\
...
```{r}
crd_dfw %>% 
  group_by(nrate_kgha,krate_kgha) %>% 
  summarise(mean(yield_kgha))
```

em stands for expected marginal means
